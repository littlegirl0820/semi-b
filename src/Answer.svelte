<script>
  ...
  export let turns;
  export let userName;
  export let playMode;  // 0: local, 1: online
  ...
  function sendAnswer() {
    if (answer === '') {
      ...
      if (turn === turns) {
        if (playMode === 1) isWaitingResult = true;
...
<main>
  {#if playMode === 0 || userName === currentPlayer}
    <div class='toparea'>
    <h1>TURN {turn}</h1>
    {#if turn === 1}
...
<style>
  ...
  */
  ...
  /* 以下、https://photopizza.design/css_loading/ より引用 */
  .wrapper{
    width:200px;
    height:60px;
    position: absolute;
